...
export function showBookDetails(bookId) {
  ...
  const coverHTML = safeCoverId
    ? `<img class="detail-cover" src="https://covers.openlibrary.org/b/id/${safeCoverId}-M.jpg" alt="cover" loading="lazy" width="80" height="112" data-cover-fallback="swap">
  ...
  const tagChips = (book.tags || []).map(t => `<span class="detail-tag">${escHtml(t)}</span>`).join(' ');
